<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="friend_list.aspx.cs" Inherits="WebWeChat.Net.Admin.backstage.page.kfzx.friend_list" %>
    ...
    <script>
        ...
        $(function () {
            ...
            $("body").delegate(".friend_item", "click", function () {
                dialogue_load(<%=uin%>, this.id);
            });

        })
        function searchFriend() {
            friend_load(<%=uin%>, $("#key").val());
        }
        function friend_load(uin, key) {
            $.ajax({
                async: true,
                type: "POST",
                data: { "uin": uin, "key": key },
                url: "/Kfzx/GetFriendList",
                success: function (data) {
                    var data = data.data;
                    // console.log(data[1].Uin);
                    var friend_list_html = "";
                    if (data.length == 0) {
                        d_list_html = '<div style="text-align:center">好友列表为空</div>';
                    }
                    else {
                        for (var i = 0; i < data.length; i++) {
                            friend_list_html += '<div  style="cursor:pointer" class="mt-comments friend_item" id="' + data[i].Uin + '">'
                                ...
                                + '<img src="/Handlers/HeadImgHandler.ashx?tb=Customer&uin=' + data[i].Uin + '" width="50px" height="50px" />'
                                + '</div>'
        ...
        function dialogue_load(uin, customer_uin) {
            $.ajax({
                async: true,
                type: "POST",
                data: { "uin": uin, "customer_uin": customer_uin },
                url: "/Kfzx/GetDialogueList",
                success: function (data) {
                    var data = data.data;
                    var d_list_html = "";
                    // console.log(data[1].Uin);
